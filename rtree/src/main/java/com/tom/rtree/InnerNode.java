...
public class InnerNode<T> extends RTreeNode<T> implements Node<T> {
  ...
  public Rectangle getBounds() {
    return bounds.orElse(Rectangle.IDENTITY);
  }
   ...
   * @return
   */
  public Collection<Rectangle> collectGrids(Collection<Rectangle> list) {
    list.add(getBounds());
    int size = children.size();
    for (int i = 0; i < size; i++) {
   ...
   */
  @Override
  public Set<T> getVisibleElements(Set<T> visibleElements, Rectangle shape) {
