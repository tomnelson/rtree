...
public interface Node<T> extends TreeNode, Bounded {
  ...
  Collection<Rectangle> collectGrids(Collection<Rectangle> list);

  Set<T> getVisibleElements(Set<T> visibleElements, Rectangle shape);
  ...
  static double overlap(Rectangle left, Rectangle right) {
    return area(left.intersect(right));
  }
